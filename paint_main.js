const path = require("path"); // HTTP 서버(express) 생성 및 구동
// 1. express 객체 생성
...
const HTTPServer = app.listen(30001, '192.168.0.23', ()=>{
...
const webSocketServer = new wsModule.Server(
  ...
  webSocketServer.on('connection', (ws, request)=>{
    ...
    usernum = num;
    console.log(`새로운 클라이언트[${ip}] 접속(${num}번)`);
    num++;
    // 2) 클라이언트에게 메시지 전송
  ...
  // 데이터 전송
  //ws.send(`클라이언트[ss]에게 전송한 메세지 : ss`);
}
   // 3) 클라이언트로부터 메시지 수신 이벤트 처리
   ws.on('message', (msg)=>{ console.log(`클라이언트[${ip}](${usernum}번)에게 수신한 메시지 : ${msg}`);
